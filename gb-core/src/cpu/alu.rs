...
impl<T: Interface> Cpu<T> {
    pub(crate) fn alu_sub(&mut self, value: u8, carry: bool) -> u8 {
        ...
        self.registers.flag(CpuFlag::H,  (self.registers.a & 0xf).wrapping_sub(value & 0xf).wrapping_sub(cy) & (0xf + 1) != 0);
        self.registers.flag(CpuFlag::C, (self.registers.a as u16) < (value as u16) + (cy as u16));
    ...
    pub(crate) fn alu_rl<F>(&mut self, value: u8, reset_zero: bool, carry: F) -> u8
        ...
        self.registers.flag(CpuFlag::Z, new_value == 0 && !reset_zero);
