...
impl SquareChannel {
    fn new(blip: Blip, with_sweep: bool) -> SquareChannel {
        SquareChannel {
            active: false,
            dac_enabled: false,
            duty: 1,
    ...
    fn run(&mut self, start_time: u32, end_time: u32) {
        if !self.active || self.period == 0 {
            if self.last_amp != 0 {
                self.blip.add_delta_fast(start_time, -self.last_amp);
                self.last_amp = 0;
                self.delay = 0;
            }
        }
            ...
            while time < end_time {
                ...
                if amp != self.last_amp {
                    self.blip.add_delta_fast(time, amp - self.last_amp);
                    self.last_amp = amp;
                }
                time += self.period;
...
impl WaveChannel {
    fn new(blip: Blip, dmg_mode: bool) -> WaveChannel {
        WaveChannel {
            active: false,
            dac_enabled: false,
...
impl Sound {
    ...
    fn new_internal(player: Box<dyn AudioPlayer>, dmg_mode: bool) -> Sound {
        ...
        let output_period = ((OUTPUT_SAMPLE_COUNT as u64 * CLOCKS_PER_SECOND as u64) / player.samples_rate() as u64) as u32;
