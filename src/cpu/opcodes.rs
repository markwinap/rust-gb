...
impl<T: Interface> Cpu<T> {
    ...
    pub fn cb_decode_execute(&mut self) -> ((Step, u16), u8) {
        match self.op_code {
            ...
            0x75 => (self.bit(6, L), 8),
            0x7d => (self.bit(7, L), 8),
            0x46 => (self.bit(0, Addr::HL), 16),
            0x4e => (self.bit(1, Addr::HL), 16),
            0x56 => (self.bit(2, Addr::HL), 16),
            0x5e => (self.bit(3, Addr::HL), 16),
            0x66 => (self.bit(4, Addr::HL), 16),
            0x6e => (self.bit(5, Addr::HL), 16),
            0x76 => (self.bit(6, Addr::HL), 16),
            0x7e => (self.bit(7, Addr::HL), 16),
    ...
    pub fn daa(&mut self) -> (Step, u16) {
        ...
        if !self.registers.flags.n {
            ...
            carry = true;
            self.registers.a = self.registers.a.wrapping_add(if self.registers.flags.h { 0x9a } else { 0xa0 })
    ...
    {
        let value = self.read_8(io8);
        let result = self.alu_rr(value, false, |flags, _| flags.c as u8);
